...
const FeatureTab = (tab) => {
    ...
    useEffect(() => {
        ...
        const cirumference = height * 2 + width * 2

        const times = [0,width / cirumference, (width + height) / cirumference,(width * 2 + height)/cirumference,1]
    ...
    const handleTabHover = () => {
        ...
        player.stop();   // more reliable reset
        player.play();
    }
    return (
        <div 
        ref={tabRef}
        onMouseEnter={handleTabHover} className="relative border lg:flex-1 border-white/15 flex rounded-xl p-2.5 gap-2.5 items-center">
            <motion.div
            style={{
                maskImage
            }} className="absolute -m-px rounded-lg inset-0 border border-[#A369FF] "></motion.div>
            <div
                className="size-12 border border-white/15 rounded-lg inline-flex items-center justify-center">
                <DotLottieReact
